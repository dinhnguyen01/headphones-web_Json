window.onload = function() {
    ...
    var numBanner = 6; // Số lượng hình banner
    ...
    // Thêm danh sách hãng tai nghe
    var company = ["Apple.jpg", "Ava.jpeg", "Awei.jpg", "Beats.jpg", "Belkin.jpg", "Jabra.jpg",
        "JBL.jpg", "Kanen.jpg", "LG.png", "Logitech.jpg", "Oppo.jpg", "Rapoo.png",
        "Samsung.png", "Sony.jpg", "Xiaomi.png"
    ];
    for (var c of company) {
        addCompany("img/headphone-company/" + c, c.slice(0, c.length - 4).replace('.', ''));
    } 
    ...
    if (filters.length) { // có filter
        ...
        // console.log(sanPhamPhanTrang);
        // console.log(filtersFromUrl.page);
        // console.log(filtersFromUrl.sort);
    ...
    addPricesRange(1000000, 2000000);
    addPricesRange(2000000, 0);
...
function phanTich_URL(filters, saveFilter) {
    var result = copyObject(listProducts);
...
function priceToString(min, max) {
    if (min == 0) {
        if(max > 1E6) 
          return 'Dưới ' + max / 1E6 + ' triệu'; 
       else 
          return 'Dưới ' + Intl.NumberFormat("de-DE").format(max) + 'đ';
    } 

    else if (max == 0) {
        if(min > 1E6) 
          return 'Trên ' + min / 1E6 + ' triệu'; 
       else 
          return 'Dưới ' + Intl.NumberFormat("de-DE").format(min) + 'đ';
    } 

    else {
        if (min < 1E6) {
            if (max < 1E6) {
                return 'Từ ' + Intl.NumberFormat("de-DE").format(min) + 'đ' + ' - ' + Intl.NumberFormat("de-DE").format(max) + 'đ';
            } else {
                return 'Từ ' + Intl.NumberFormat("de-DE").format(min) + 'đ' + ' - ' + max / 1E6 + ' triệu';
            }
        } 
